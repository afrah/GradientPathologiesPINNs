...
class Klein_Gordon:
    ...
    def __init__(self, layers, operator, ics_sampler, bcs_sampler, res_sampler, 
        ...
        self.loss_ic_u = self.loss_functionlog((self.u_ics_tf - self.u_ics_pred))
        self.loss_ic_u_t = self.loss_functionlog((self.u_t_ics_pred))
        self.loss_bc1 = self.loss_functionlog((self.u_bc1_pred - self.u_bc1_tf))
        self.loss_bc2 = self.loss_functionlog((self.u_bc2_pred - self.u_bc2_tf))

        self.loss_bcs =  (self.loss_ic_u + self.loss_bc1 + self.loss_bc2)

        # Residual loss
        self.loss_res = self.loss_functionlog((self.r_pred - self.r_tf))

        # Total loss
        self.loss =  self.adaptive_constant_res_tf *self.loss_res +  self.adaptive_constant_bcs_tf * self.loss_bcs + self.adaptive_constant_ics_tf * self.loss_ic_u_t
        ...
        self.adaptive_constant_bcs_val_log = []
        self.adaptive_constant_ics_val_log = []
        self.adaptive_constant_res_val_log = []
        ...
        self.t_r_ntk_tf = tf.placeholder(tf.float32, shape=(self.kernel_size, 1))
        self.x_r_ntk_tf = tf.placeholder(tf.float32, shape=(self.kernel_size, 1))

        self.bc_ntk_pred = self.net_u(self.t_bc_ntk_tf , self.x_bc_ntk_tf)
        self.ic_ntk_pred = self.net_u(self.t_ic_ntk_tf , self.x_ic_ntk_tf)
        ...
        self.J_bc = self.compute_jacobian(self.bc_ntk_pred) 
        self.J_ic = self.compute_jacobian(self.ic_ntk_pred) 
        self.J_r = self.compute_jacobian(self.r_ntk_pred)
        
        # The empirical NTK = J J^T, compute NTK of PINNs 
        self.K_bc = self.compute_ntk(self.J_bc, self.t_bc_ntk_tf, self.J_bc, self.t_bc_ntk_tf)
        self.K_ic = self.compute_ntk(self.J_ic, self.t_ic_ntk_tf, self.J_ic, self.t_ic_ntk_tf)
        self.K_rr = self.compute_ntk(self.J_r, self.t_r_ntk_tf, self.J_r, self.t_r_ntk_tf)
        
        # Initialize Tensorflow variables
        init = tf.global_variables_initializer()
        self.sess.run(init)

    def loss_functionlog(self , x):
        return (tf.abs(tf.log(tf.reduce_mean(tf.square(x)) + 1.0)))   
    ...
    def train(self, nIter=10000, batch_size=128):
        ...
        for it in range(nIter):
            ...
            if it % 1000 == 0:
                print("Compute NTK...")
                X_ics_batch, _ = self.fetch_minibatch(self.ics_sampler, self.kernel_size)
                X_bc1_batch, _ = self.fetch_minibatch(self.bcs_sampler[0], self.kernel_size)
                X_bc2_batch, _ = self.fetch_minibatch(self.bcs_sampler[1], self.kernel_size)

                X_bc_batch = np.concatenate([X_bc1_batch , X_bc2_batch] , 0)
                idx_c = np.random.choice( X_bc_batch.shape[0],  self.kernel_size, replace=False)
                # Fetch residual mini-batch
                X_res_batch, _ = self.fetch_minibatch(self.res_sampler,  self.kernel_size)

                # Define a dictionary for associating placeholders with data
                tf_dict_ntk = {self.t_bc_ntk_tf: X_bc_batch[idx_c, 0:1],
                                self.x_bc_ntk_tf: X_bc_batch[idx_c, 1:2],
                                self.t_ic_ntk_tf: X_ics_batch[:, 0:1],
                                self.x_ic_ntk_tf: X_ics_batch[:, 1:2], 
                                  self.t_r_ntk_tf: X_res_batch[:, 0:1], 
                                    self.x_r_ntk_tf: X_res_batch[:, 1:2]
                }
                            
                K_bc, K_ic , K_rr = self.sess.run([self.K_bc ,   self.K_ic , self.K_rr], tf_dict_ntk)
                summation = K_bc +  K_ic  +  K_rr
                self.adaptive_constant_res_val =( np.trace(summation) /  np.trace(K_rr)) if np.trace(K_rr)!= 0.0 else 1.0
                self.adaptive_constant_bcs_val =( np.trace(summation) /  np.trace(K_bc)) if np.trace(K_bc)!= 0.0 else 1.0
                self.adaptive_constant_ics_val =( np.trace(summation) /  np.trace(K_ic)) if np.trace(K_ic)!= 0.0 else 1.0

    
                self.print('adaptive_constant_bc_val: {:.3e}'.format( self.adaptive_constant_bcs_val))
                self.print('adaptive_constant_res_val: {:.3e}'.format( self.adaptive_constant_res_val))
                self.print('adaptive_constant_ics_val_log: {:.3e}'.format( self.adaptive_constant_ics_val))

                self.adaptive_constant_bcs_val_log.append(self.adaptive_constant_bcs_val)
                self.adaptive_constant_ics_val_log.append(self.adaptive_constant_ics_val)
                self.adaptive_constant_res_val_log.append(self.adaptive_constant_res_val)
    ...
    def make_output_dir(self , PATH):
        ...
        shutil.copyfile('/home/vlq26735/code/PhD/GradientPathologiesPINNs/src/cases/Klein_Gordon/ntk.py', os.path.join(dirname, 'ntk.py'))
    ...
    def plot_lambda(self ):
        ...
        ax.semilogy(self.mean_grad_res_log, label=r'$\bar{\nabla_\theta {u_{phy}}}$' , color = 'tab:blue')
        ax.semilogy(self.mean_grad_bcs_log, label=r'$Max{\nabla_\theta {u_{bc}}}$' , color = 'tab:green')
        ax.semilogy(self.mean_grad_ic_ut_log, label=r'$Max{\nabla_\theta {u_{ic}}}$' , color = 'tab:red')
        ...
        # fig, ax = plt.subplots()
        # fig.set_size_inches([15,8])
        ax2.semilogy(self.adaptive_constant_res_val_log, label=r'$\bar{\lambda_{phy}}$'  ,  linestyle='dashed' , color = 'tab:blue') 
        ax2.semilogy(self.adaptive_constant_bcs_val_log, label=r'$\bar{\lambda_{bc}}$'  ,  linestyle='dashed' , color = 'tab:green') 
        ax2.semilogy(self.adaptive_constant_ics_val_log, label=r'$\bar{\lambda_{ic}}$'  ,  linestyle='dashed' , color = 'tab:red') 
