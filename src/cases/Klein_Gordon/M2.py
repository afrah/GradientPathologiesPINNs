...
class Klein_Gordon:
    ...
    def __init__(self, layers, operator, ics_sampler, bcs_sampler, res_sampler, 
        ...
        for i in range(len(self.layers) - 1):
            self.mean_grad_res_list.append(tf.reduce_max(tf.abs(self.grad_res[i]))) 
        ...
        self.mean_grad_res = tf.reduce_max(tf.stack(self.mean_grad_res_list))
    ...
    def train(self, nIter=10000, batch_size=128):
        ...
        for it in range(nIter):
            ...
            if it % 1000 == 0:
                ...
                beta = 0.9
                self.adaptive_constant_ics_val = (1.0 - beta) * self.adaptive_constant_ics_val + beta *(mean_grad_res / mean_grad_ics)
                self.adaptive_constant_bcs_val = (1.0 - beta) * self.adaptive_constant_bcs_val + beta *(mean_grad_res / mean_grad_bcs)
    ...
    def make_output_dir(self , PATH):
        ...
        shutil.copyfile('/home/vlq26735/code/PhD/GradientPathologiesPINNs/src/cases/Klein_Gordon/M2.py', os.path.join(dirname, 'M2.py'))
