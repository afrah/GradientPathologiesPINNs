...
class Helmholtz2D:
    def __init__(self, layers, operator, ics_sampler, bcs_sampler, res_sampler, lam, mode ,  starter_learning_rate , PATH , sess ):
        ...
        # Adaptive constant
        self.beta = 0.9
        self.adaptive_constant_val = np.array(100.0)
        ...
        # Total loss
        self.loss = self.loss_res + self.adaptive_constant_tf * self.loss_bcs
 ...
 ############################################################
    ...
    def train(self, nIter=10000, batch_size=128):
        ...
        for it in range(nIter):
            ...
            if it % 1000 == 0:
                ...
                self.adaptive_constant_val = (1.0 - beta) * self.adaptive_constant_val + beta * (mean_grad_res / mean_grad_bcs)
                self.print('adaptive_constant_val: {:.3e}'.format( self.adaptive_constant_val))
    ...
    def plot_lambda(self ):
        ...
        ax2.semilogy(self.adpative_constant_log, label=r'$\bar{\lambda_{bc}}$'  ,  linestyle='dashed' , color = 'tab:red') 
