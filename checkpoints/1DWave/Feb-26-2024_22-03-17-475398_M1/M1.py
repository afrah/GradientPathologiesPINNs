...
class PINN:
    ...
    def __init__(self, layers, ics_sampler, bcs_sampler, res_sampler, c , mode ,  PATH , sess ):
        # Normalization 
    ...
    def trainmb(self, nIter=10000, batch_size=128, log_NTK=False, update_lam=False):
        ...
        for it in range(nIter):
            ...
            if it % 100 == 0:
                ...
                self.adaptive_constant_res_val = adaptive_constant_res_val * ( 1.0 - self.rate) + self.rate * self.adaptive_constant_res_val
                ...
                self.print('max_grad_res/mean_grad_ics: {:.3e}'.format( max_grad_res/mean_grad_ics))

                self.print('mean_grad_bcs/max_grad_res: {:.3e}'.format( mean_grad_bcs/max_grad_res))
                self.print('mean_grad_bcs/mean_grad_ics: {:.3e}'.format( mean_grad_bcs/mean_grad_ics))

                self.print('mean_grad_ics/max_grad_res: {:.3e}'.format( mean_grad_ics/max_grad_res))
                self.print('mean_grad_ics/mean_grad_bcs: {:.3e}'.format( mean_grad_ics/mean_grad_bcs))


                self.print('adaptive_constant_res_val: {:.3e}'.format( adaptive_constant_res_val))
                self.print('adaptive_constant_ics_val: {:.3e}'.format( adaptive_constant_ics_val))
                self.print('madaptive_constant_bcs_val: {:.3e}'.format( adaptive_constant_bcs_val))
