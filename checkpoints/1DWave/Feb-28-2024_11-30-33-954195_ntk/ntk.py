...
class PINN:
    ...
    def __init__(self, layers, ics_sampler, bcs_sampler, res_sampler, c , mode ,  starter_learning_rate , PATH , sess ):
        ...
        self.loss_ics_u =  self.loss_functionlog(self.u_ics_tf - self.u_ics_pred)
        self.loss_ics_u_t =self.loss_functionlog(self.u_t_ics_pred)
        self.loss_bc1 =self.loss_functionlog(self.u_bc1_pred)
        self.loss_bc2 =self.loss_functionlog(self.u_bc2_pred)

        self.loss_bcs = self.loss_ics_u + self.loss_bc1 + self.loss_bc2

        # Residual loss
        self.loss_res =self.loss_functionlog(self.r_pred)
        ...
        self.ic_ntk_pred = self.net_u(self.x_ic_ntk_tf , self.x_ic_ntk_tf )
        ...
        self.K_bc = self.compute_ntk(self.J_bc, self.x_bc_ntk_tf, self.J_bc, self.x_bc_ntk_tf)
        self.K_ic = self.compute_ntk(self.J_ic, self.x_ic_ntk_tf, self.J_ic, self.x_ic_ntk_tf)
    ...
    def loss_functionlog(self , x):
        return (tf.log(tf.reduce_mean(tf.square(x)) + 1.0))
    ...
    def trainmb(self, nIter=10000, batch_size=128, log_NTK=False, update_lam=False):
        ...
        for it in range(nIter):
            ...
            if it % 1000 == 0:
                ...
                bcs_batch = np.concatenate([X_bc1_batch, X_bc2_batch],0)
                idx_c = np.random.choice( bcs_batch.shape[0],  self.kernel_size, replace=False)


                tf_dict_ntk = {self.t_bc_ntk_tf: bcs_batch[idx_c, 0:1],
                               self.x_bc_ntk_tf: bcs_batch[idx_c, 1:2], 
                ...
                K_bc, K_rr ,  K_ic= self.sess.run([self.K_bc ,   self.K_rr,   self.K_ic], tf_dict_ntk)
                summation = K_bc +   K_rr + K_ic
                ...
                self.adaptive_constant_ics_val =( np.trace(summation) /  np.trace(K_ic)) if np.trace(K_bc)!= 0.0 else 1.0
    ...
    def make_output_dir(self , PATH):
        ...
        shutil.copyfile('/home/vlq26735/code/PhD/GradientPathologiesPINNs/src/cases/Wave/ntk.py', os.path.join(dirname, 'ntk.py'))
