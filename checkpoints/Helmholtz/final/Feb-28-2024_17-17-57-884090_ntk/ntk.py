...
class Helmholtz2D:
    def __init__(self, layers, operator, ics_sampler, bcs_sampler, res_sampler, lam, mode ,  starter_learning_rate , PATH , sess ):
        ...
        # Boundary loss
        self.loss_bc1 =self.loss_functionlog((self.u_bc1_tf - self.u_bc1_pred))
        self.loss_bc2 =self.loss_functionlog((self.u_bc2_tf - self.u_bc2_pred))
        self.loss_bc3 =self.loss_functionlog((self.u_bc3_tf - self.u_bc3_pred))
        self.loss_bc4 =self.loss_functionlog((self.u_bc4_tf - self.u_bc4_pred))
        self.loss_bcs = (self.loss_bc1 + self.loss_bc2 + self.loss_bc3 + self.loss_bc4)

        # Residual loss
        self.loss_res =self.loss_functionlog((self.r_tf - self.r_pred))

        # Total loss
        self.loss = self.adaptive_constant_res_tf * self.loss_res + self.adaptive_constant_bc_tf * self.loss_bcs
        ...
        self.x1_u_ntk_tf = tf.placeholder(tf.float32, shape=(self.kernel_size, 1))
        self.x2_u_ntk_tf = tf.placeholder(tf.float32, shape=(self.kernel_size, 1))
        self.x1_r_ntk_tf = tf.placeholder(tf.float32, shape=(self.kernel_size, 1))
        self.x2_r_ntk_tf = tf.placeholder(tf.float32, shape=(self.kernel_size, 1))

        self.u_ntk_pred = self.net_u(self.x1_u_ntk_tf , self.x2_u_ntk_tf)
        self.r_ntk_pred = self.net_r(self.x1_r_ntk_tf , self.x2_r_ntk_tf)
        ...
        self.K_uu = self.compute_ntk(self.J_u, self.x1_u_ntk_tf, self.J_u, self.x1_u_ntk_tf)
        self.K_rr = self.compute_ntk(self.J_r, self.x1_r_ntk_tf, self.J_r, self.x1_r_ntk_tf)
 ...
 ############################################################
    ...
    def train(self, nIter=10000, batch_size=128):
        ...
        for it in range(nIter):
            ...
            tf_dict = {self.x1_bc1_tf: X_bc1_batch[:, 0:1],
                       self.x2_bc1_tf: X_bc1_batch[:, 1:2],
            ...
            if it % 100 == 0:
                ...
                X_bc3_batch, _ = self.fetch_minibatch(self.bcs_sampler[2],  self.kernel_size)
                X_bc4_batch, _ = self.fetch_minibatch(self.bcs_sampler[3],  self.kernel_size)
                ...
                # Define a dictionary for associating placeholders with data
                tf_dict_ntk = {self.x1_u_ntk_tf: X_bc_batch[idx_c, 0:1],
                                self.x2_u_ntk_tf: X_bc_batch[idx_c, 1:2],
                            self.x1_r_ntk_tf: X_res_batch[:, 0:1], 
                            self.x2_r_ntk_tf: X_res_batch[:, 1:2]
                }
                            
                K_uu_value, K_rr_value = self.sess.run([self.K_uu ,   self.K_rr], tf_dict_ntk)
                summation = K_uu_value +   K_rr_value
                self.adaptive_constant_res_val =( np.trace(summation) /  np.trace(K_rr_value)) if np.trace(K_rr_value)!= 0.0 else 1.0
                self.adaptive_constant_bc_val =( np.trace(summation) /  np.trace(K_uu_value)) if np.trace(K_uu_value)!= 0.0 else 1.0

    
                self.print('adaptive_constant_bc_val: {:.3e}'.format( self.adaptive_constant_bc_val))
    ...
    def make_output_dir(self , PATH):
        ...
        shutil.copyfile('/home/vlq26735/code/PhD/GradientPathologiesPINNs/src/cases/Helmholtz/ntk.py', os.path.join(dirname, 'ntk.py'))
    ...
    def plot_lambda(self ):
        ...
        ax.semilogy(self.mean_grad_res_log, label=r'$\bar{\nabla_\theta {u_{phy}}}$' , color = 'tab:green')
        ax.semilogy(self.mean_grad_bcs_log, label=r'$Max{\nabla_\theta {u_{bc}}}$' , color = 'tab:red')
        ...
        # fig, ax = plt.subplots()
        # fig.set_size_inches([15,8])
        ax2.semilogy(self.adpative_constant_res_log, label=r'$\bar{\lambda_{res}}$'  ,  linestyle='dashed' , color = 'tab:green') 
        ax2.semilogy(self.adpative_constant_bc_log, label=r'$\bar{\lambda_{bc}}$'  ,  linestyle='dashed' , color = 'tab:red') 
