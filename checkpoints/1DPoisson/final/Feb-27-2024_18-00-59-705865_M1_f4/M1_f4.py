...
class Possion_M1_f4():
    def __init__(self, layers, X_u, Y_u, X_r, Y_r ,mode , starter_learning_rate , PATH ,  sess):
        ...
        # Boundary loss
        self.loss_bcs = self.loss_function4(self.u_bc_pred - self.u_bc_tf)

        # Residual loss        
        self.loss_res =  self.loss_function4(self.r_tf - self.r_pred)
    ...
    def loss_function4(self , x):
        delta = 1.0
        xSquare = tf.square(x)
        return (tf.math.reduce_sum(tf.math.log(1.0 + xSquare) - (xSquare / (2.0 * xSquare + 2.0))))
    ...
    def trainmb(self, nIter=10000, batch_size=128, log_NTK=True, log_weights=True):
        ...
        for it in range(nIter):
            ...
            if it % 1000 == 0:
                ...
                if it  == 0:
                    self.lam_bc = 1.0 #summation / (mean_grad_bcs  +1e-8 )
                    self.lam_res = 1.0 #summation / (mean_grad_res  +1e-8 )
 ...
 ############################################################
  ...
  ############################################################
    ...
    def make_output_dir(self , PATH):
        ...
        shutil.copyfile('/home/vlq26735/code/PhD/GradientPathologiesPINNs/src/cases/Poisson/M1_f4.py', os.path.join(dirname, 'M1_f4.py'))
