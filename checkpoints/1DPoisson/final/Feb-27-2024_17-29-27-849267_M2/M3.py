...
class Possion_M3():
    def __init__(self, layers, X_u, Y_u, X_r, Y_r ,mode , starter_learning_rate , PATH ,  sess):
        ...
        # Boundary loss
        self.loss_bcs = self.loss_functionlog(self.u_bc_pred - self.u_bc_tf)
    ...
    def trainmb(self, nIter=10000, batch_size=128, log_NTK=True, log_weights=True):
        ...
        for it in range(nIter):
            ...
            if it % 1000 == 0:
                ...
                if it  == 0:
                    summation = mean_grad_bcs + mean_grad_res
                    self.lam_bc = summation / (mean_grad_bcs  +1e-8 )
                    self.lam_res = summation / (mean_grad_res  +1e-8 )

                self.print("self.lam_bc: " ,  self.lam_bc)    
                self.print("self.lam_res : " , self.lam_res )   
                self.adaptive_constant_bcs_log.append(self.lam_bc)
                self.adaptive_constant_res_log.append(self.lam_res)
 ...
 ############################################################
  ...
  ############################################################
    ...
    def plot_lambda(self ):
        ...
        ax2.semilogy(self.adaptive_constant_res_log, label=r'$\bar{\lambda_{res}}$'  ,  linestyle='dashed' , color = 'tab:red') 
    ...
    def make_output_dir(self , PATH):
        ...
        shutil.copyfile('/home/vlq26735/code/PhD/GradientPathologiesPINNs/src/cases/Poisson/M3.py', os.path.join(dirname, 'M3.py'))
