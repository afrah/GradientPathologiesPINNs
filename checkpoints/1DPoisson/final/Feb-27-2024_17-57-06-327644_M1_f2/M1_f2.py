...
class Possion_M1_f2():
    def __init__(self, layers, X_u, Y_u, X_r, Y_r ,mode , starter_learning_rate , PATH ,  sess):
        ...
        # Boundary loss
        self.loss_bcs = self.loss_function2(self.u_bc_pred - self.u_bc_tf)

        # Residual loss        
        self.loss_res =  self.loss_function2(self.r_tf - self.r_pred)
    ...
    def loss_function2(self , x):
        delta = 1.0
        xSquare = tf.square(x)
        return tf.math.reduce_sum((2.0 * xSquare) /(xSquare + 4* delta * delta))
 ...
 ############################################################
  ...
  ############################################################
    ...
    def make_output_dir(self , PATH):
        ...
        shutil.copyfile('/home/vlq26735/code/PhD/GradientPathologiesPINNs/src/cases/Poisson/M1_f2.py', os.path.join(dirname, 'M1_f2.py'))
